...
(function(window, document) {
  ...
  var transformProp = 'transform' in document.body.style ? 'transform' : 'webkitTransform',
  ...
  Dragble.prototype = {
    init: function() {
      var config = this.config,
          axis = config.axis ? config.axis.toUpperCase() : '';
      this.ele = typeof this.attachTo == 'string' ? document.querySelector(this.attachTo) : this.attachTo;
      this.handle = typeof config.handle == 'string' ? document.querySelector(config.handle) : config.handle || this.ele;
      this.ele.dragble = this;
      this.ele.onChange = this.onChange;
      this.ele.restrictX = axis == 'X';
      this.ele.restrictY = axis == 'Y';
      if(config.limits) {
        config.limitsX = axis ? config.limits : config.limits[0];
        config.limitsY = axis ? config.limits : config.limits[1];
      }
      this.limitsX = config.limitsX || [-Infinity, Infinity];
      this.limitsY = config.limitsY || [-Infinity, Infinity];
      this.ele.snapBack = config.snapBack || false;
      if (this.containment) {
    ...
    dragStart: function(e) {
      var self = this,
          ele = self.ele,
          restrictX = self.restrictX,
          restrictY = self.restrictY,
          limitsX = self.limitsX,
          limitsY = self.limitsY,
    ...
    contain: function(element) {
      ...
      if (ele) {
        ...
        this.limitsX = [xLimit1, xLimit2];
        this.limitsY = [yLimit1, yLimit2];
